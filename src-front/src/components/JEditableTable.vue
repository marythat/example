...
<div>
    ...
    <div v-if="actionButton" class="action-button">
      ...
      <el-button type="primary" icon="el-icon-plus" @click="handleClickAdd">新增</el-button>
      <span class="gap"></span>
...
<!--        <a-popconfirm
        ...
        <el-popconfirm :title="`确定要删除这 ${selectedRowIds.length} 项吗?`" @onConfirm="handleConfirmDelete">
          <el-button type="primary" icon="el-icon-minus" slot="reference">删除</el-button>
        </el-popconfirm>
        <template v-if="showClearSelectButton">
          <span class="gap"></span>
          <!--<a-button icon="delete" @click="handleClickClearSelect">清空选择</a-button>-->
          <el-button icon="el-icon-delete" @click="handleClickClearSelect">清空选择</el-button>
        </template>
      </template>
    </div>
                ...
                <div v-if="rowSelection" class="td td-cb" :style="style.tdLeft">
                  ...
                  <template v-for="(id,i) in [`${row.id}`]">
                    <el-checkbox
                      ...
                      :name="id"
                      :key="i"
                      v-model="selectedRowIds.indexOf(id) !== -1"
...
<!--                    <template v-else-if="col.type === formTypes.select">
                      ...
                      <span
                        ...
                        >
                          ...
                          &lt;!&ndash;<template v-for="(opt,optKey) in col.options">&ndash;&gt;
                          &lt;!&ndash;<a-select-option :value="opt.value" :key="optKey">{{ opt.title }}</a-select-option>&ndash;&gt;
...
&lt;!&ndash;                        <a-input
                          ...
                          :value="file.name"
                        >&ndash;&gt;
                        &lt;!&ndash;inputValues[rowIndex][col.key]&ndash;&gt;
                          <j-upload
                            :value="file"
                            :id="id"
                            @change="handleChangeUpload"
                          >
                            <a-button> <a-icon type="upload" />{{ col.placeholder }}</a-button>
                          </j-upload>
&lt;!&ndash;                          <template slot="addonBefore" style="width: 30px">
...
<script>
  // import JUpload from '@/components/jeecg/JUpload1'
  // import Vue from 'vue'
  import Draggable from 'vuedraggable'
  // import { ACCESS_TOKEN } from '@/store/mutation-types'
  import { FormTypes, VALIDATE_NO_PASSED, cloneObject, randomString } from './JEditableTableUtil'
  ...
  export default {
    name: 'JEditableTable',
    // components: { JDate, Draggable, JUpload },
    ...
    data() {
      return {
        ...
        // 行数据
        rows: [],
        // 行高，height + padding + bogetElementsByClassNamerder
        rowHeight,
        // 滚动条顶部距离
        scrollTop: 0,
        // 绑定 select 的值
...
/*              } else if (column.type === FormTypes.select) {
                ...
                if (sourceValue) {
                  this.uploadValues[inputId] = sourceValue
                } else {
                  this.uploadValues[inputId] = ""
                }*/
...
/*              if (!edited && this.jdateValues.hasOwnProperty(modelKey)) {
      ...
      /** 左侧行选择框change事件 */
      handleChangeLeftCheckbox(event,e) {
        let { name } = e.target

        if ((this.disabledRowIds || []).indexOf(name) !== -1) {
          return
        }

        let index = this.selectedRowIds.indexOf(name)
        if (index !== -1) {
          this.selectedRowIds.splice(index, 1)
        } else {
          this.selectedRowIds.push(name)
        }
